...
**Grep-шаблон:** `bool _audioEnabled = false;`
**Файлы для проверки:** `lib/home_screen.dart`
**Проверка:** grep не находит объявление `bool _audioEnabled = false;` (игнорируется сброс при закрытии видео)
**Дата исправления:** 2026-06-15
**Контекст:** `_audioEnabled` по умолчанию было `false`. Пользователь загружал видео со звуком, а экспорт выходил без звука. Исправлено на `true`.
