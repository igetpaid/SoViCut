...
class ConcatStrategy implements ExportStrategy {
  ...
  Future<bool> export({
    ...
    final int audioBitrate = exportSettings?.audioBitrate ?? (originalBitrate ~/ 1000);
    ...
    // Phase 1: extract each clip to temp file
    ...
    for (int i = 0; i < activeClips.length; i++) {
      ...
      if (clipResult != 0) {
        print('Error extracting clip ${i+1}');
        success = false;
        break;
      }
    ...
    // Phase 2: concatenate temp files
